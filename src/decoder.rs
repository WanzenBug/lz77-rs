...
pub struct Lz77Decoder<R> where R: io::Read {
    inner: R,
...
impl<R> Lz77Decoder<R> where R: io::Read {
    ...
    fn read_token(&mut self) -> io::Result<CodeWord> {
        ...
        let mut cw = CodeWord::new(16 - self.options.window_size).expect("Misaligned length for codewords");
        cw.read(&token_buf[..]);
        Ok(cw)
    }
}

impl<R> io::Read for Lz77Decoder<R> where R: io::Read {
